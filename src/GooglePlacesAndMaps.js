...
export default function GooglePlacesAndMaps({ itineraryId }) {
    ...
    try {
      ...
      if (user) {
        ...
        if (itineraryDoc.exists() && countryItineraryDoc.exists()) {
          ...
          // Check the current number of locations
          const locations = currentData.locations || [];
          const countryLocations = currentDataCountry.locations || [];

          if (locations.length >= 15) {
            console.error('Cannot add more locations: limit of 15 reached for this day');
            return; // Stop execution if the limit is reached
          }

          if (countryLocations.length >= 15) {
            console.error('Cannot add more locations: limit of 15 reached for this day in the country itinerary');
            return; // Stop execution if the limit is reached
          }
  ...
  return (
    <div className='itinerary-day-details-card-container'>
      ...
      <div className='google-maps-import'>
        <GoogleMap
